...
use super::{Attester, TeeEvidence, InitDataResult};
use anyhow::*;
use base64::Engine;
use log::info;
...
impl Attester for TpmAttester {
    ...
    async fn get_runtime_measurement(&self, pcr_index: u64) -> Result<Vec<u8>> {
        ...
        let target_pcr = pcrs.get(idx).ok_or_else(|| anyhow!("Register index out of bounds"))?;
...
impl TpmAttester {
    pub async fn get_evidence_async(&self, report_data: Vec<u8>) -> anyhow::Result<serde_json::Value> {
        <Self as super::Attester>::get_evidence(self, report_data).await
    }
} 