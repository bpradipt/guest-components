...
//!     --no-default-features --features pull-debug-bin,oci-client-rustls,signature-cosign-rustls
//!
//! Run:
...
async fn main() {
    ...
    match result {
        Ok(image_info) => {
            ...
            info!("Config digest:   {}", image_info.config_digest);
            info!("Manifest digest: {}", image_info.manifest_digest);
            if !args.skip_verify {
                verify_bundle(&bundle_dir);
            }
            // Print the config digest to stdout so it can be captured by scripts
            println!("{}", image_info.config_digest);
        }
        Err(e) => {
